...
- **APIs/Services**: Google Reviews API, Google Maps API, Pocket Pay (payment gateway), ImprovMX (email forwarding for collaboration form).
   ...
   **Phase 10 — Customer profiles + Branch CRUD.** First foundation slice for the eventual CRM (per the user's mock). No schema change — uses existing `customers`, `cars`, `orders`, `branches`.

   Backend (`server/routes.ts`):
   - `GET  /api/admin/customers?search=&branch_id=&page=&per_page=` (owner + manager) — paginated customer list joined with vehicle count, lifetime visits, lifetime spend, and last-visit timestamp. Search matches name / phone / plate (≥ 2 chars). Branch filter narrows to customers who have ever transacted at that branch.
   - `GET  /api/admin/customers/:id` — full profile: customer record, all vehicles (with per-vehicle visit count + spend), the last 100 orders (with branch + staff names), and lifetime stats (visits, refunds, spend, first/last visit, distinct branch count).
   - `PATCH /api/admin/customers/:id` — edit name + notes. Phone is treated as the immutable identity (matches POS upsert behaviour).
   - `GET  /api/admin/branches/full` (owner + manager) — branch list with full columns + staff/order counts per branch. Replaces the old name+location-only `GET /api/admin/branches` for management UI; the lighter endpoint stays for dropdowns.
   - `POST /api/admin/branches` (owner) and `PATCH /api/admin/branches/:id` (owner) — full branch CRUD over name, location, google_maps_url, embed_url, review_url, is_open. No DELETE — orders FK to branches, so closing via `is_open = false` is the safe path.

   Frontend:
   - `client/src/components/admin/CustomersTab.tsx` — new component (split out of admin.tsx to keep that file under 3k lines). Two-column layout: left is the searchable, paginated table (name, phone, vehicles, visits, lifetime spend, last visit); right is a sticky profile panel with edit-in-place name+notes, KPI tiles (lifetime spend, visits, first visit, branch count), vehicle cards (per-vehicle spend + visits), and a scrollable recent-visits list with refund highlighting.
   - `client/src/components/admin/BranchesTab.tsx` — new component, owner-only. Card grid showing each branch with status badge (Open/Closed), staff count, order count, and external links to map + reviews. "New branch" + "Edit" buttons open a unified Dialog with form validation (URL fields require `http(s)://`) and an Open-today switch.
   - `client/src/pages/admin.tsx` — registered the two new tabs in the visible-tab order list (Customers for manager+owner, Branches for owner only). Also fixed an old visibleTabs miss where "trends" wasn't counted in the grid columns.
   - `client/src/pages/pos.tsx` — added a "View profile" link next to the customer Remove button in the POS customer-info form. Only renders when (a) staff role is owner/manager and (b) the matched vehicle resolves to a known customer. Opens `/admin?tab=customers&id=…` in a new tab so the cashier doesn't lose their in-progress sale. Also surfaces "N previous visits · B$X lifetime" under the customer inputs as light context for the cashier.
