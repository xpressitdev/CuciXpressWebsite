...
- **APIs/Services**: Google Reviews API, Google Maps API, Pocket Pay (payment gateway), ImprovMX (email forwarding for collaboration form).
   ...
   **Phase 8 polish — Home page design-system pass.** The landing page was the last surface still mixing three style vocabularies (soft shadcn shadows, partial brutalist, fully brutalist). All seven home sections rewritten to share the Phase 7 vocabulary while keeping copy + behavior intact:
   - `Hero` — added eyebrow "Cuci Xpress · Brunei drive-thru", tightened headline to `font-extrabold tracking-tight`, replaced inline brutalist motion-buttons with `.cuci-cta` (purple primary, orange secondary), wrapped the hero video in 2px-black + 4px shadow, replaced the floating "Guaranteed" tile with `.cuci-card`.
   - `Stats` — tiles rewritten with `.cuci-kpi`, icons sit in 2px-black-bordered colored squares (instead of round soft circles), eyebrow + duotone "Our success **story**" headline, value digits use `font-extrabold tracking-tight`.
   - `Gallery` — eyebrow + duotone "See our **work**" headline, every image now wrapped in 2px-black + 4px brutalist frame for visual rhythm with the rest of the page.
   - `LocationsWithTestimonials` — `LocationCard` rewritten as `.cuci-card` (was soft shadow); selected state is 2px purple ring + ring-offset (was inset ring); flagship badge is now a hard-cornered orange tag with black border; service chips are mini bordered pills; testimonials wrapped in `.cuci-card` (sticky on lg) with `.cuci-card-soft` rows. Reviews container moved out of soft `bg-gray-100` into the system.
   - `ServicePricing` — three plan tiles rewritten as `.cuci-card` with brutalist icon squares + dashed-divider price block; Most-Popular badge upgraded to bordered pill; CTAs use `.cuci-cta` (white / cuci-primary / cuci-secondary). Payment-methods grid now sits inside a `.cuci-card` with mini `.cuci-card-soft` tiles per method. "Ready to get started" footer rewritten as a single `.cuci-card` with three brutalist CTAs (queue / locations / call).
   - `PricingSection` — dropped `rounded-full` shadcn pills for `.cuci-cta` rectangles; added eyebrow + duotone "Looking for **subscriptions?**" headline.
   - `Investment` — normalized `border-3` → `border-2` (was the only spot in the app using border-3, breaking the brutalist rhythm); inputs/select/textarea now wear `border-2 border-black`; benefit rows use `.cuci-card-soft`; submit CTA uses `.cuci-cta`; section bg flipped to `bg-gray-50` to break up the white run between ServicePricing → PricingSection → Investment.
   - All section headings now share the same recipe: `cuci-eyebrow` label + `text-4xl md:text-5xl font-extrabold tracking-tight` h2 with one keyword in `text-cuci-primary`. Section bg alternation (white / gray-50) preserved for vertical rhythm — only the *vocabulary* was unified.
